function [out, interp_mask] = interpolate_all_nans(data)
...
%       interp_mask - binary mask, 1 = interpolated, 0 = untouched
    ...
    for k = 1:numel(start_idx)
        ...
        if isempty(left_vals)
            left_anchor_val = data(find(~isnan(data(1:s-1)),1,'last'));
        ...
        if isempty(right_vals)
            right_anchor_val = data(find(~isnan(data(e+1:end)),1,'first')+e);
            right_anchor_idx = find(~isnan(data(e+1:end)),1,'first')+e;
        else
            right_anchor_val = mean(right_vals);
            right_anchor_idx = e+1;
        end
        
        % Edge case: if one side is completely missing, use the other side’s value
        if isempty(left_anchor_val)
            left_anchor_val = right_anchor_val;
            left_anchor_idx = s-1;
        end
        if isempty(right_anchor_val)
            right_anchor_val = left_anchor_val;
            right_anchor_idx = e+1;
        end
        
        % Interpolate linearly across gap
